...
function apply(target, { value: { move_target: given_move_target = undefined, onmovestart = undefined, onmoveend = undefined, snap_back = false } = {} }) {
	...
	function start_move(/** @type {MouseEvent | TouchEvent} */ move_event) {
		...
		let mouse_start_x = /** @type {MouseEvent} */(move_event).pageX || /** @type {TouchEvent} */(move_event).changedTouches[0]?.pageX || 0
		let mouse_start_y = /** @type {MouseEvent} */(move_event).pageY || /** @type {TouchEvent} */(move_event).changedTouches[0]?.pageY || 0
		...
		function on_mousemove(/** @type {MouseEvent | TouchEvent} */ mouse_event) {
			...
			window.requestAnimationFrame(() => {
				offset_x = (/** @type {MouseEvent} */(mouse_event).pageX || /** @type {TouchEvent} */(mouse_event).changedTouches?.[0]?.pageX || 0) - mouse_start_x
				offset_y = (/** @type {MouseEvent} */(mouse_event).pageY || /** @type {TouchEvent} */(mouse_event).changedTouches?.[0]?.pageY || 0) - mouse_start_y
