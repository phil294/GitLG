...
export default defineComponent({
	...
	setup(props) {
		let refs_elems
		let padding_left = 5
		let lines = computed(() => props.commit.vis_lines.map((vis_line) => ({
			...
			vis_line,
			style: { stroke: vis_line.branch?.color },
			class: { is_head: vis_line.branch?.id === head_branch.value },
		...
		let circle = computed(() => {
			if (! branch_line.value || ! props.commit.branch)
				return null; return {
				style: { stroke: props.commit.branch.color },
				class: { is_head: props.commit.branch.id === head_branch.value },

				// I guess this could also be calculated more elegantly, but this kind of

				// approximation seems to be good enough for all cases

				cx: padding_left + (branch_line.value.vis_line.x0 + branch_line.value.vis_line.xn + (branch_line.value.vis_line.xcs || 0) + (branch_line.value.vis_line.xce || 0)) / 4 * vis_v_width.value,
				cy: props.height * 0.5,
				r: 4,
			}
		})
		refs_elems = computed(() => ({
			...
			style: { left: (circle.value?.cx || padding_left) + vis_v_width.value - 2 + 'px' },
		}))
