...
export default
	...
	setup: ->
		...
		log_action =
			...
			args: "log --graph --oneline --date=iso-local --pretty={EXT_FORMAT} -n 15000 --skip=0 --all --color=never --invert-grep --extended-regexp --grep=\"^untracked files on \" --grep=\"^index on \""
			options: [
