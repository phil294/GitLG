...
function parse(log_data, branch_data, stash_data, separator, curve_radius) {
	let author_email, author_name, branch_line, hash_long, iso_datetime, last_vis_line, ref1, ref2, ref3, ref4, ref5, ref6, ref_name, refs, refs_csv, remote_branch_name_parts, remote_name, stash, subject, subject_merge_match, w_char_match, wrong_branch_match, wrong_branch_matches, x, y, z
	...
	for (branch_line of branch_data.split('\n')) {
   ...
   * @type {Record<string, VisLine>}  */
	...
	for (let [row_no, row] of Object.entries(rows)) {
		...
		refs = refs_csv.split(', ').map((r) => r.split(' -> ')[1] || r).filter((r) => r !== 'refs/stash').filter(is_truthy).map((id) => {
			let ref
		...
		}).filter(is_truthy).sort(git_ref_sort)

		let branch_tips = refs.filter(is_branch)
       ...
       * @type VisLine  */
			...
			case '|':
				if (v_n?.branch)
					...
					v_branch = v_ne?.branch
				vis_line = { x0: 0.5, xn: 0.5, yn: 0.5 }; break
			case '_':
				v_branch = v_ee?.branch
				vis_line = { x0: 1, xn: 0 }; break
			case '/':
				if (v_ne?.char === '*')
					v_branch = v_ne?.branch
				else if (v_ne?.char === '|')
					...
					v_branch = v_n?.branch
				vis_line = { x0: 1, xn: -0.5, xce: 0.5 }; break
			case '\\':
				if (v_e?.char === '|')
					v_branch = v_e?.branch
				else if (v_w_char === '|') {
		...
		if (subject) {
			...
			for (let [branch_id, vis_line] of Object.entries(densened_vis_line_by_branch_id)) {
				if (vis_line.y0 == null)
					vis_line.y0 = 0; if (vis_line.yn == null)
					vis_line.yn = 1; if (vis_line.xce == null)
				...
				// upwards splitting effect
					...
					vis_line.xce = vis_line.xn; if (vis_line.yce == null)
			...
			commits.push({
				...
				vis_lines: Object.values(densened_vis_line_by_branch_id).sort((a, b) => (b.xcs || 0) + (b.xce || 0) - (a.xcs || 0) - (a.xce || 0)),
	...
	for (branch of branches)
		branch.color = (function() {
			...
			case 'stage': case 'staging': case 'production': return '#d7d700'; default:
	...
	).sort(git_ref_sort).slice(0, 10000)
